    ...
    /*!
    * tiptap-extensions v1.35.2
    * (c) 2024 überdosis GbR (limited liability)
    * @license MIT
    */
  
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('tiptap'), require('tiptap-commands'), require('lowlight/lib/core'), require('prosemirror-view'), require('tiptap-utils'), require('prosemirror-model'), require('prosemirror-state'), require('prosemirror-tables'), require('prosemirror-transform'), require('prosemirror-collab'), require('prosemirror-history')) :
  typeof define === 'function' && define.amd ? define(['exports', 'tiptap', 'tiptap-commands', 'lowlight/lib/core', 'prosemirror-view', 'tiptap-utils', 'prosemirror-model', 'prosemirror-state', 'prosemirror-tables', 'prosemirror-transform', 'prosemirror-collab', 'prosemirror-history'], factory) :
  (global = global || self, factory(global.tiptapExtensions = {}, global.tiptap, global.tiptapCommands, global.low, global.prosemirrorView, global.tiptapUtils, global.prosemirrorModel, global.prosemirrorState, global.prosemirrorTables, global.prosemirrorTransform, global.prosemirrorCollab, global.prosemirrorHistory));
}(this, (function (exports, tiptap, tiptapCommands, low, prosemirrorView, tiptapUtils, prosemirrorModel, prosemirrorState, prosemirrorTables, prosemirrorTransform, prosemirrorCollab, prosemirrorHistory) { 'use strict';
  ...
  function getDecorations({
    ...
    const decorations = [];
    const blocks = tiptapUtils.findBlockNodes(doc).filter(item => item.node.type.name === name);
    ...
    return new tiptap.Plugin({
      ...
      state: {
        ...
        apply: (transaction, decorationSet, oldState, newState) => {
          ...
          const oldNodes = tiptapUtils.findBlockNodes(oldState.doc).filter(item => item.node.type.name === name);
          const newNodes = tiptapUtils.findBlockNodes(newState.doc).filter(item => item.node.type.name === name); // Apply decorations if selection includes named node, or transaction changes named node.
  ...
  class Mention extends tiptap.Node {
    ...
    insertMention(range, attrs, schema) {
      ...
      return tiptapCommands.replaceText(range, nodeType, attrs, nodeFragment);
    }
    ...
    commands({
      ...
      return {
        createTable: ({
          ...
          const nodes = tiptapUtils.createTable(schema, rowsCount, colsCount, withHeaderRow);
        ...
        setCellAttr: ({
          name,
          value
        }) => prosemirrorTables.setCellAttr(name, value),
